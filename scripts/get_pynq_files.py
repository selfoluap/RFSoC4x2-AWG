...
def main():
    parser = argparse.ArgumentParser(
        description="Find and copy Vivado .bit and .hwh files for PYNQ overlays."
    )
    parser.add_argument(
        "project_dir",
        nargs="?",
        metavar="PROJECT_DIR",
        help="Root directory of the Vivado project. If omitted, use -i/--input.",
    )
    parser.add_argument(
        "-i", "--input",
        dest="input_dir",
        help="Root directory of the Vivado project.",
    )
    parser.add_argument(
        "-o", "--output",
        default=Path.cwd(),
        help="Destination directory for the copied files (default: current working directory).",
    )
    parser.add_argument(
        "-r", "--run-time",
        default="impl_1",
        help="Implementation run name (default: impl_1).",
    )

    args = parser.parse_args()

    project_dir = args.project_dir or args.input_dir
    if not project_dir:
        print("Error: No project directory specified. Provide it as a positional argument or via -i.", file=sys.stderr)
        sys.exit(1)
    ...
    bit_file, hwh_file = find_vivado_files(project_path, args.run_time)
